...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.EIH = EIH(d_model)
        self.DS = DomainSynergy(2*d_model)
    @staticmethod
    ...
    def forward(self, x: torch.Tensor, **kwargs):          #64
        ...
        x_EIH = self.EIH(x) 
        x_EIH = x_fft.permute(0, 2, 3, 1).contiguous() 
        x = self.act(self.conv2d(x)) # (b, d, h, w)                   #2*128*128*128
        x_ss2d = self.forward_core(x)
        x_ss2d = x_ss2d.permute(0, 3, 1, 2)
        x_EIH = x_EIH.permute(0, 3, 1, 2)    
        y=self.DS(x_ss2d,x_fft)          #
        y = y.permute(0, 2, 3, 1)
        y = y * F.silu(z)
        out = self.out_proj(y)   
...
class DomainSynergy(nn.Module):
    def __init__(self, in_channels):
        super(DomainSynergy, self).__init__()
